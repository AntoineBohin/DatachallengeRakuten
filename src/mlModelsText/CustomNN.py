...
def train_model(model_path=DEFAULT_MODEL_PATH,build_new_nn=BUILD_NEW_NN):
    if build_new_nn:
        build_custom_nn()
    ...
    y_encoded = lb.fit_transform(train_csv["ProductTypeCode"][0:80000])
...
def evaluate_model(Tfidf_matrix_to_evaluate):
    ...
    if not b==VOCABULARY_LENGTH:
        ...
        for k in range(80001,84915):
            y_test = estimator.predict(X_tfidf_sample[k].toarray())
            y_classes = y_test.argmax(axis=-1)
            y_pred = lb.inverse_transform(y_classes)
            number_of_predictions+=1
            if y_pred[0]==train_csv["ProductTypeCode"][k]:
                good_predictions+=1
            print(k,y_pred,train_csv["ProductTypeCode"][k])
